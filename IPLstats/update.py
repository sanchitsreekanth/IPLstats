...
def update(matches_in_year):
	...
	for i,match_id in enumerate(p):
		...
		batting = batting_scorecard(series_id,match_id,year)
		bowling = bowling_scorecard(series_id,match_id,year)
		for ele in batting:
			for i in ele:
	...
	if player_dict:
		with open(cwd + os.path.sep + 'data' + os.path.sep+'player_ids.csv','a') as f:
			writer = csv.writer(f)
